...
class OKXPaperTrader:
    ...
    def __init__(
        ...
        log_dir: str = "results/paper_trading",
        max_stop_pct: float = 0.017,  # BTC 最大止损 1.7%
    ):
        self.symbol = symbol
    ...
    def check_fractal_signal(self, m30_df: pd.DataFrame, h1_df: pd.DataFrame) -> dict:
        ...
        if signal == "long":
            ...
            if risk <= 0:
                return None
            # 最大止损截断
            if hasattr(self, 'max_stop_pct') and self.max_stop_pct:
    ...
    def _smart_sleep_seconds(self, failed: bool = False) -> float:
        """
        计算下一次检测的等待秒数。
        目标：每10分钟检测一次（00, 10, 20, 30, 40, 50分 + 30秒偏移）。
        如果刚失败，则60秒后重试。
        """
        if failed:
            return 60
    ...
    def run(self):
        ...
        print(f"最大止损: {self.max_stop_pct*100:.1f}%")
...
if __name__ == "__main__":
    ...
    ensure_dir("results/paper_trading")
