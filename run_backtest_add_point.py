...
迭代回测：最优加仓点位置
=================================================
基于最新策略：分型(5,2) + 1h宽松共振 + 4h严格共振 + RR=1 + 100x杠杆 + 冷却3根

研究：从入场点到止损点区间内，浮亏到哪个位置是最优加仓点。

加仓点参数 add_frac（浮亏占风险区间的比例）：
  做多加仓价 = 入场价 - add_frac * (入场价 - 止损价)
  add_frac = 0.25 → 浮亏1/4，距离止损还有3/4
  add_frac = 0.50 → 浮亏一半（入场和止损中间）
  add_frac = 0.75 → 浮亏3/4，距离止损还剩1/4
  add_frac = 0.90 → 浮亏90%，很接近止损

加仓后：平均成本摊低，总止损不变，止盈按平均成本重算(RR=1)
"""
import sys
import os
...
def run_backtest(m30, h1, h4, asset_cfg, add_frac=0.0):
    """
    add_frac=0.0 表示不加仓；>0 表示在浮亏 add_frac 比例处加仓
    """
    ...
    while i < max_i:
        if in_pos:
            ...
            if pos_side == "long":
                ...
                elif add_frac > 0 and not added and bar["low"] <= add_price:
                    # 触发加仓（此时 low > sl，因为上面已判止损）
                    added = True
                ...
                if bar["high"] >= sl:
                    exit_flag, exit_price = "loss", sl
                elif add_frac > 0 and not added and bar["high"] >= add_price:
                    added = True
                    avg_entry = (entry1 + add_price) / 2
...
def main():
    print("=" * 100)
    print("迭代回测：最优加仓点位置（基于 4h严格共振策略）")
    print("加仓点 = 入场价 - add_frac × 风险区间，add_frac=0表示不加仓")
    print("=" * 100)

    # 加仓点档位
    add_frac_list = [0.0, 0.25, 0.40, 0.50, 0.60, 0.75, 0.85, 0.90, 0.95]
    ...
    for name, cfg in ASSETS.items():
        ...
        print(f"\n  {'加仓点(浮亏%)':<16}{'交易':>6}{'胜率':>9}{'累计收益':>18}{'盈亏因子':>10}{'最大回撤':>11}{'加仓次数':>9}{'加仓胜率':>9}")
        print("  " + "-" * 95)

        for add_frac in add_frac_list:
            trades = run_backtest(m30, h1, h4, cfg, add_frac=add_frac)
            s = compute_stats(trades)
            label = "不加仓" if add_frac == 0 else f"浮亏{int(add_frac*100)}%"
            print(f"  {label:<16}{s['trades']:>6}{s['win_rate']*100:>8.1f}%"
